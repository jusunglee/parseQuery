...
import { Book, Logger } from './types.js';
...
function pickColumns<T extends keyof Book>(
  books: Book[],
  columns: T[]
): Array<Pick<Book, T>> {
  return books.map((book) => {
    const partial = {} as Pick<Book, T>;
...
function isBookKeyArray(keys: string[]): keys is (keyof Book)[] {
...
function parseQuery(logger: winston.Logger, query: string) {
  const parser =
    /^SELECT (.*) FROM [a-z]+(?: WHERE (.*?))?(?: ORDER BY (.*?))?$/;
  const matches = parser.exec(query);
  if (matches === null) {
    throw new Error('null matches');
  }

  const qualifyingRows: Book[] = [];
...
function rowSatisfiesWhere(
  ...
  for (const where of whereParts) {
    ...
    const column = Object.keys(book).find((colName) =>
      where.startsWith(colName)
    );
    if (column === undefined) {
      throw new Error(
        `where clause ${where} does not start with a valid column`
      );
    }
    ...
    if (operator === undefined) {
      throw new Error(
        `where clause ${afterColumn} does not have a supported operator`
      );
    }
    ...
    if (
      ...
      logger.debug(
        `not satisfied ${operator}, ${JSON.stringify(book)}, ${column}, ${afterOperator}`
      );
      return false;
    }
  }
...
function evaluateWhereOperation(
  logger: Logger,
  args: {
    operator: string;
    book: Book;
  ...
  const columnVal = book[columnName as keyof Book];
      ...
      switch (typeof columnVal) {
        ...
        case 'string':
          const trimQuotes = String(
            afterOperator.substring(1, afterOperator.length - 1)
          );
          logger.debug(
            `equality operation: '${columnVal}' vs '${trimQuotes})}'`
          );
          return columnVal === trimQuotes;
        default:
          throw new Error(
            `error doing operator evaluation for =, type ${typeof columnVal} not supported`
          );
      }
    case '>':
      return book[columnName as keyof Book] > afterOperator;
    case '<':
      return book[columnName as keyof Book] > afterOperator;
    default:
